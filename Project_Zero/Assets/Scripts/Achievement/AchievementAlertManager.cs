...
public class AchievementAlertManager : MonoBehaviour
{
    ...
    public void Update()
    {
        if ((!isDisplaying) && (achieveAlertQueue.Count > 0))
        {
            ...
            SpriteRenderer illust = alertPanel.GetChild(0).GetChild(0).GetComponent<SpriteRenderer>();
            ...
            targetY = startPos.y - alertPanel.localScale.y * 3 / 2;
        }
    }
    ...
    public void FixedUpdate()
    {
        ...
        {
            if (panelProgress < 30)
                alertPanel.position = new Vector3(startPos.x, startPos.y - alertPanel.localScale.y / 20 * panelProgress, 0);
            else if (panelProgress == 90)
            {
                ...
                alertPanel.position = new Vector3(startPos.x, targetY + alertPanel.localScale.y / 20 * (panelProgress - 60), 0);
            panelProgress++;
        }
    }
}
